...
* DNS configuration for this IP (`mugle1.example.com` -> `3.3.3.3`)
...
Uncomment and update the following lines in wallet config (by default `~/.mugle/mugle-wallet.toml`):
...
If you have Stratum server enabled (you run a miner) make sure that wallet listener URL starts with `https` in node config (by default `~/.mugle/mugle-server.toml`):

```toml
wallet_listener_url = "https://mugle1.example.com:16815"
```

Make sure your user has read access to the files (see below for how to do it). Restart wallet. If you changed your node configuration restart `mugle` too. When you (or someone else) send mugles to this wallet the destination (`-d` option) must start with `https://`, not with `http://`.
...
```sh
sudo certbot certonly --standalone -d mugle1.example.com
```
...
Congratulations! Your certificate and chain have been saved at:
  /etc/letsencrypt/live/mugle1.example.com/fullchain.pem
 Your key file has been saved at:
  /etc/letsencrypt/live/mugle1.example.com/privkey.pem
...
### Change permissions
Now you have the certificate files but only root user can read it. We run mugle as `ubuntu` user. There are different scenarios how to fix it, the simplest one is to create a group which will have access to `/etc/letsencrypt` directory and add our user to this group.
...
```toml
tls_certificate_file = "/etc/letsencrypt/live/mugle1.example.com/fullchain.pem"
tls_certificate_key =  "/etc/letsencrypt/live/mugle1.example.com/privkey.pem"
```
