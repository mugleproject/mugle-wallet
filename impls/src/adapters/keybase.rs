...
const SLATE_NEW: &str = "mugle_slate_new";
const SLATE_SIGNED: &str = "mugle_slate_signed";
...
fn poll(nseconds: u64, channel: &str) -> Option<Slate> {
	...
	error!(
		"No response from @{} in {} seconds. Mugle send failed!",
		channel, nseconds
	);
	None
}
	...
	if keybase_notify_ttl > 0 {
		...
		if let Ok(username) = my_username {
			...
			if vec.len() > 1 {
				...
				info!(
					"tx from @{} is done, please check on mugle wallet. tx uuid: {}",
					sender, tx_uuid,
				);
			}
		} else {
