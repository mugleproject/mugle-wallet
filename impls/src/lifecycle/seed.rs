...
impl WalletSeed {
	...
	pub fn recover_from_phrase(
		...
		if !Path::new(&data_file_dir).exists() {
			return Err(ErrorKind::WalletDoesntExist(
				...
				"To create a new wallet from a recovery phrase, use 'mugle-wallet init -r'"
					.to_owned(),
			)
			.into());
		}
	...
	pub fn from_file(
		...
		if Path::new(seed_file_path).exists() {
			...
			error!(
				"wallet seed file {} could not be opened (mugle-wallet init). \
				 Run \"mugle-wallet init\" to initialize a new wallet.",
				seed_file_path
			);
