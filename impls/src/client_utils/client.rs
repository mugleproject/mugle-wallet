// Copyright 2018 The Mugle Developers
//
...
impl Client {
	...
	fn build(socks_proxy_addr: Option<SocketAddr>) -> Result<Self, Error> {
		...
		headers.insert(USER_AGENT, HeaderValue::from_static("mugle-client"));
	...
	fn build_request(
		...
		if let Some(api_secret) = api_secret {
			let basic_auth = format!("Basic {}", to_base64(&format!("mugle:{}", api_secret)));
