...
impl HTTPNodeClient {
	/// Create a new client that will communicate with the given mugle node
	pub fn new(
		node_url: &str,
		node_api_secret: Option<String>,
...
impl NodeClient for HTTPNodeClient {
	...
	fn get_outputs_from_node(
		...
		let chunk_size = match env::var("MUGLE_OUTPUT_QUERY_SIZE") {
			Ok(s) => match s.parse::<usize>() {
				Ok(c) => c,
				Err(e) => {
					error!(
						"Unable to parse MUGLE_OUTPUT_QUERY_SIZE, defaulting to {}",
						chunk_default
					);
					error!("Reason: {}", e);
					chunk_default
