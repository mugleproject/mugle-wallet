...
use crate::mugle_core::core::amount_to_hr_string;
use crate::mugle_core::core::transaction::{
	FeeFields, Input, Inputs, KernelFeatures, NRDRelativeHeight, Output, OutputFeatures,
	Transaction, TxKernel, Weighting,
};
use crate::mugle_core::libtx::{aggsig, build, proof::ProofBuild, tx_fee};
use crate::mugle_core::map_vec;
use crate::mugle_keychain::{BlindSum, BlindingFactor, Keychain, SwitchCommitmentType};
use crate::mugle_util::secp::key::{PublicKey, SecretKey};
use crate::mugle_util::secp::pedersen::Commitment;
use crate::mugle_util::secp::Signature;
use crate::mugle_util::{secp, static_secp_instance};
...
use crate::slate_versions::{CURRENT_SLATE_VERSION, MUGLE_BLOCK_HEADER_VERSION};
use crate::Context;
...
pub struct VersionCompatInfo {
	...
	pub version: u16,
	/// The mugle block header version this slate is intended for
	...
	pub fn blank(num_participants: u8, is_invoice: bool) -> Slate {
		...
		Slate {
			...
			version_info: VersionCompatInfo {
				...
				block_header_version: MUGLE_BLOCK_HEADER_VERSION,
			},
			payment_proof: None,
