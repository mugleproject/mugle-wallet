...
use crate::mugle_core::core::transaction::{FeeFields, OutputFeatures};
use crate::mugle_core::ser as mugle_ser;
use crate::mugle_core::ser::{Readable, Reader, Writeable, Writer};
use crate::mugle_keychain::BlindingFactor;
use crate::mugle_util::secp::key::PublicKey;
use crate::mugle_util::secp::pedersen::{Commitment, RangeProof};
use crate::mugle_util::secp::Signature;
...
impl Writeable for SlateStateV4 {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), mugle_ser::Error> {
		let b = match self {
...
impl Readable for SlateStateV4 {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateStateV4, mugle_ser::Error> {
...
impl Readable for UuidWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<UuidWrap, mugle_ser::Error> {
...
impl Readable for SlateOptFields {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptFields, mugle_ser::Error> {
...
impl Readable for SigsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<SigsWrap, mugle_ser::Error> {
...
impl Readable for SlateOptStructs {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptStructs, mugle_ser::Error> {
		let status = reader.read_u8()?;
...
impl Readable for ComsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ComsWrap, mugle_ser::Error> {
...
impl Readable for ProofWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ProofWrap, mugle_ser::Error> {
...
impl serde::Serialize for SlateV4Bin {
	...
	{
		let mut vec = vec![];
		mugle_ser::serialize(&mut vec, mugle_ser::ProtocolVersion(4), self)
		...
		D: serde::Deserializer<'de>,
	{
		...
		impl<'de> serde::de::Visitor<'de> for SlateV4BinVisitor {
				...
				E: serde::de::Error,
			{
				...
				let s = mugle_ser::deserialize(
					&mut reader,
					mugle_ser::ProtocolVersion(4),
					mugle_ser::DeserializationMode::default(),
				)
...
impl Readable for SlateV4Bin {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateV4Bin, mugle_ser::Error> {
...
fn slate_v4_serialize_deserialize() {
	use crate::mugle_util::from_hex;
	use crate::mugle_util::secp::key::PublicKey;
	use crate::Slate;
	use mugle_wallet_util::mugle_core::global::{set_local_chain_type, ChainTypes};
	use mugle_wallet_util::mugle_keychain::{ExtKeychain, Keychain, SwitchCommitmentType};
	...
	let mut vec = Vec::new();
	let _ = mugle_ser::serialize_default(&mut vec, &v4_bin).expect("serialization failed");
	let b4_bin_2: SlateV4Bin = mugle_ser::deserialize_default(&mut &vec[..]).unwrap();
