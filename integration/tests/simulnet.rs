...
extern crate mugle_apiwallet as apiwallet;
extern crate mugle_libwallet as libwallet;
extern crate mugle_refwallet as wallet;
extern crate mugle_wallet_config as wallet_config;
#[macro_use]
extern crate log;
...
fn simulate_full_sync() {
	...
	let test_name_dir = "mugle-sync";
	framework::clean_all_output(test_name_dir);

	let s1 = servers::Server::new(framework::config(1000, "mugle-sync", 1000)).unwrap();
	...
	let s2 = servers::Server::new(framework::config(1001, "mugle-sync", 1000)).unwrap();
...
fn simulate_fast_sync() {
	...
	let test_name_dir = "mugle-fast";
	framework::clean_all_output(test_name_dir);

	// start s1 and mine enough blocks to get beyond the fast sync horizon
	let s1 = servers::Server::new(framework::config(2000, "mugle-fast", 2000)).unwrap();
	let stop = Arc::new(Mutex::new(StopState::new()));
	s1.start_test_miner(None, stop.clone());
	...
	let mut conf = config(2001, "mugle-fast", 2000);
...
fn simulate_long_fork() {
	...
	let test_name_dir = "mugle-long-fork";
	framework::clean_all_output(test_name_dir);
...
fn long_fork_test_preparation() -> Vec<servers::Server> {
	...
	let mut conf = framework::config(2100, "mugle-long-fork", 2100);
	conf.archive_mode = Some(false);
	...
	for i in 1..6 {
		let mut conf = config(2100 + i, "mugle-long-fork", 2100);
		conf.archive_mode = Some(false);
		conf.api_secret_path = None;
...
/// Intended to replicate https://github.com/mugleproject/mugle/issues/1325
#[ignore]
#[test]
