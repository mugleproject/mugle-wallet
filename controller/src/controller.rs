...
use crate::libwallet::{
	...
	WalletLCProvider, MUGLE_BLOCK_HEADER_VERSION,
};
use crate::util::secp::key::SecretKey;
use crate::util::{from_hex, static_secp_instance, to_base64, Mutex};
use failure::ResultExt;
use futures::channel::oneshot;
use mugle_wallet_api::JsonId;
use mugle_wallet_config::types::{TorBridgeConfig, TorProxyConfig};
...
lazy_static! {
	pub static ref MUGLE_OWNER_BASIC_REALM: HeaderValue =
		HeaderValue::from_str("Basic realm=MugleOwnerAPI").unwrap();
}
...
fn check_middleware(
	...
	match name {
		...
		_ => {
			...
			if let Some(s) = slate {
				if bhv > 4 && s.version_info.block_header_version < MUGLE_BLOCK_HEADER_VERSION {
	...
	K: Keychain + 'static,
{
	...
	if api_secret.is_some() {
		let api_basic_auth =
			"Basic ".to_string() + &to_base64(&("mugle:".to_string() + &api_secret.unwrap()));
		let basic_auth_middleware = Arc::new(BasicAuthMiddleware::new(
			api_basic_auth,
			&MUGLE_OWNER_BASIC_REALM,
