...
//! Mugle wallet command-line function implementations
	...
	controller::owner_single_use(None, keychain_mask, Some(owner_api), |api, m| {
		if args.estimate_selection_strategies {
			let strategies = vec!["smallest", "all"]
				...
				.collect::<Result<Vec<_>, mugle_wallet_libwallet::Error>>()?;
	...
	controller::owner_single_use(None, keychain_mask, Some(owner_api), |api, m| {
		if args.estimate_selection_strategies {
			...
			slate = match result {
				Ok(s) => {
					info!(
						"Invoice processed: {} mugle (strategy '{}')",
