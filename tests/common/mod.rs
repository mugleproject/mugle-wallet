...
extern crate mugle_wallet;

use mugle_wallet_config as config;
use mugle_wallet_impls::test_framework::LocalWalletClient;
...
use mugle_wallet_api::{EncryptedRequest, EncryptedResponse, JsonId};
use mugle_wallet_config::{GlobalWalletConfig, WalletConfig, MUGLE_WALLET_DIR};
use mugle_wallet_impls::{DefaultLCProvider, DefaultWalletImpl};
use mugle_wallet_libwallet::{NodeClient, WalletInfo, WalletInst};
use mugle_wallet_util::mugle_core::global::{self, ChainTypes};
use mugle_wallet_util::mugle_keychain::ExtKeychain;
use mugle_wallet_util::mugle_util::{from_hex, static_secp_instance};
use util::secp::key::{PublicKey, SecretKey};

use mugle_wallet::cmd::wallet_args;
...
macro_rules! setup_proxy {
	($test_dir: expr, $chain: ident, $wallet1: ident, $client1: ident, $mask1: ident, $wallet2: ident, $client2: ident, $mask2: ident) => {
		...
		// wallet init
		let arg_vec = vec!["mugle-wallet", "-p", "password", "init", "-h"];
		// should create new wallet file
		let $client1 = LocalWalletClient::new("wallet1", wallet_proxy.tx.clone());

		let target = std::path::PathBuf::from(format!("{}/wallet1/mugle-wallet.toml", $test_dir));
		println!("{:?}", target);
		if !target.exists() {
		...
		let target = std::path::PathBuf::from(format!("{}/wallet2/mugle-wallet.toml", $test_dir));
		if !target.exists() {
...
pub fn config_command_wallet(
	dir_name: &str,
	wallet_name: &str,
) -> Result<(), mugle_wallet_controller::Error> {
	let mut current_dir;
	...
	config_file_name.push("mugle-wallet.toml");
	if config_file_name.exists() {
		return Err(mugle_wallet_controller::ErrorKind::ArgumentError(
			"mugle-wallet.toml already exists in the target directory. Please remove it first"
				.to_owned(),
		))?;
	}
...
> {
	...
	// legacy hack to avoid the need for changes in existing mugle-wallet.toml files
	// remove `wallet_data` from end of path as
	// new lifecycle provider assumes mugle_wallet.toml is in root of data directory
	let mut top_level_wallet_dir = PathBuf::from(wallet_config.clone().data_file_dir);
	if top_level_wallet_dir.ends_with(MUGLE_WALLET_DIR) {
...
pub fn execute_command(
	...
	arg_vec: Vec<&str>,
) -> Result<String, mugle_wallet_controller::Error> {
