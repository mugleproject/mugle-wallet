...
use mugle_wallet_impls::test_framework::{self, LocalWalletClient, WalletProxy};

use clap::App;
use std::thread;
use std::time::Duration;

use mugle_wallet_impls::DefaultLCProvider;
...
/// command line tests
fn command_line_test_impl(test_dir: &str) -> Result<(), mugle_wallet_controller::Error> {
	setup(test_dir);
	...
	// wallet init
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "init", "-h"];
	...
	// Create wallet 2
	let arg_vec = vec!["mugle-wallet", "-p", "password2", "init", "-h"];
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "account", "-c", "mining"];
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "account"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

	// let's see those accounts
	let arg_vec = vec!["mugle-wallet", "-p", "password2", "account"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
	...
	mugle_wallet_controller::controller::owner_single_use(
		Some(wallet1.clone()),
		mask1,
		None,
		|api, m| {
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "-a", "mining", "info"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	let arg_vec = vec!["mugle-wallet", "-a", "mining", "-p", "password1", "txs"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password2", "-a", "account_1", "info"];
	...
	let arg_vec = vec![
		...
		"send",
		"-d",
		"tmugle1ak8aaxpjg6ct5uje4lgzvjp65l0nrmgxndp5xjy74sumzp7wasysje3kmf",
		"10",
	];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "scan", "-d"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password2", "invoice", "65"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
	let file_name = format!(
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "-a", "mining", "txs"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	// txs and outputs (mostly spit out for a visual in test logs)
	let arg_vec = vec!["mugle-wallet", "-p", "password1", "-a", "mining", "outputs"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

	let arg_vec = vec!["mugle-wallet", "-p", "password2", "txs"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;

	let arg_vec = vec!["mugle-wallet", "-p", "password2", "outputs"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
	...
	let arg_vec = vec!["mugle-wallet", "-p", "password2", "txs", "-t", &tx_id[..]];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
