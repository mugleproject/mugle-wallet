...
use mugle_wallet_api::{ECDHPubkey, JsonId};
...
use mugle_wallet_libwallet::{InitTxArgs, Slate, SlateVersion, VersionedSlate};
use mugle_wallet_util::mugle_keychain::ExtKeychain;
use serde_json;

use mugle_wallet_util::mugle_util::Mutex;
...
#[test]
fn owner_v3_lifecycle() -> Result<(), mugle_wallet_controller::Error> {
	...
	let (chain, wallet2, mask2_i) = {
		...
		let arg_vec = vec!["mugle-wallet", "owner_api", "-l", "46820", "--run_foreign"];
	...
	let res = send_request(1, "http://127.0.0.1:46820/v3/owner", req)?;
	println!("RES 1: {:?}", res);
	...
	println!("RES 4: {:?}", res);
	assert!(res.is_ok());
	let pb = PathBuf::from(format!("{}/wallet1/mugle-wallet.toml", test_dir));
	assert!(pb.exists());
	...
	//15) Ask wallet 2 for some mugles
	let req = serde_json::json!({
		"jsonrpc": "2.0",
		"id": 1,
		...
		"params": {
			...
			"args": {
				"amount": "6000000000",
				"message": "geez a block of mugles",
				"dest_acct_name": null,
	...
	let res =
		send_request::<VersionedSlate>(1, "http://127.0.0.1:46820/v2/foreign", &req.to_string())?;
	println!("RES 16: {:?}", res);
	assert!(res.is_ok());
