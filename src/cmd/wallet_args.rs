...
use crate::config::MUGLE_WALLET_DIR;
...
use clap::ArgMatches;
use failure::Fail;
use mugle_wallet_api::Owner;
use mugle_wallet_config::{config_file_exists, TorConfig, WalletConfig};
use mugle_wallet_controller::command;
use mugle_wallet_controller::{Error, ErrorKind};
use mugle_wallet_impls::{DefaultLCProvider, DefaultWalletImpl};
use mugle_wallet_libwallet::{self, Slate, SlatepackAddress, SlatepackArmor};
use mugle_wallet_libwallet::{IssueInvoiceTxArgs, NodeClient, WalletInst, WalletLCProvider};
use mugle_wallet_util::mugle_core as core;
use mugle_wallet_util::mugle_core::core::amount_to_hr_string;
	...
	K: keychain::Keychain + 'static,
{
	...
	match wallet_args.subcommand() {
		...
		_ => {
			let msg = format!("Unknown wallet command, use 'mugle-wallet help' for details");
