...
//! Main for building the binary of a Mugle Reference Wallet
...
pub fn info_strings() -> (String, String) {
	(
		format!(
			"This is Mugle Wallet version {}{}, built for {} by {}.",
...
fn real_main() -> i32 {
	let yml = load_yaml!("mugle-wallet.yml");
	...
	let mut config = match config::initial_setup_wallet(&chain_type, current_dir, create_path) {
		...
		Err(e) => match e {
			ConfigError::PathNotFoundError(m) => {
				println!("Wallet configuration not found at {}. (Run `mugle-wallet init` to create a new wallet)", m);
				return 0;
			}
			m => {
				println!("Unable to load wallet configuration: {} (Run `mugle-wallet init` to create a new wallet)", m);
				return 0;
			}
		},
	};
