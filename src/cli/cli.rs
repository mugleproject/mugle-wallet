...
use mugle_wallet_config::{TorConfig, WalletConfig};
use mugle_wallet_controller::command::GlobalArgs;
use mugle_wallet_controller::Error;
use mugle_wallet_impls::DefaultWalletImpl;
use mugle_wallet_libwallet::{NodeClient, StatusMessage, WalletInst, WalletLCProvider};
...
const COLORED_PROMPT: &'static str = "\x1b[36mmugle-wallet>\x1b[0m ";
const PROMPT: &'static str = "mugle-wallet> ";
	...
	K: keychain::Keychain + 'static,
{
	...
	let yml = load_yaml!("../bin/mugle-wallet.yml");
	...
	loop {
		match reader.readline(PROMPT) {
			Ok(command) => {
				...
				// Just add 'mugle-wallet' to each command behind the scenes
				// so we don't need to maintain a separate definition file
				let augmented_command = format!("mugle-wallet {}", command);
				let args =
