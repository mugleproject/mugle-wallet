...
pub const WALLET_CONFIG_FILE_NAME: &str = "mugle-wallet.toml";
const WALLET_LOG_FILE_NAME: &str = "mugle-wallet.log";
const MUGLE_HOME: &str = ".mugle";
/// Wallet data directory
pub const MUGLE_WALLET_DIR: &str = "wallet_data";
/// Node API secret
...
fn get_mugle_path(
	...
	// Check if mugle dir exists
	let mut mugle_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	mugle_path.push(MUGLE_HOME);
	mugle_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !mugle_path.exists() && create_path {
		fs::create_dir_all(mugle_path.clone())?;
	}

	if !mugle_path.exists() {
		Err(ConfigError::PathNotFoundError(String::from(
			mugle_path.to_str().unwrap(),
		)))
	} else {
		Ok(mugle_path)
	}
...
fn check_api_secret_file(
	...
	let mugle_path = match data_path {
		Some(p) => p,
		None => get_mugle_path(chain_type, false)?,
	};
	let mut api_secret_path = mugle_path;
...
pub fn initial_setup_wallet(
	...
	// Use config file if current directory if it exists, .mugle home otherwise
	let (path, config) = if let Some(p) = check_config_current_dir(WALLET_CONFIG_FILE_NAME) {
		...
		let mugle_path = match data_path {
			Some(p) => p,
			None => get_mugle_path(chain_type, create_path)?,
		};

		// Get path to default config file
		let mut config_path = mugle_path.clone();
		...
		match config_path.clone().exists() {
			false => {
				...
				default_config.update_paths(&mugle_path);
				(mugle_path, default_config)
			}
			true => {
...
impl GlobalWalletConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalWalletConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Testnet => {
				defaults.api_listen_port = 16815;
				defaults.check_node_api_http_addr = "http://127.0.0.1:16813".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_listen_port = 26815;
				defaults.check_node_api_http_addr = "http://127.0.0.1:26813".to_owned();
			}
			_ => {}
		}
		defaults_conf
	...
	pub fn update_paths(&mut self, wallet_home: &PathBuf) {
		...
		wallet_path.push(MUGLE_WALLET_DIR);
