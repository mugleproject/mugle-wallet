...
/// * When running `mugle-wallet listen` with defaults, the V2 api is available at
/// `localhost:6815/v2/foreign`
...
pub trait ForeignRpc {
	...
	```
	# mugle_wallet_api::doctest_helper_json_rpc_foreign_assert_response!(
	# r#"
	{
		"jsonrpc": "2.0",
...
macro_rules! doctest_helper_json_rpc_foreign_assert_response {
	($request:expr, $expected_response:expr, $use_token:expr, $blocks_to_mine:expr, $init_tx:expr, $init_invoice_tx:expr) => {
		...
		use mugle_wallet_api::run_doctest_foreign;
		use serde_json;
