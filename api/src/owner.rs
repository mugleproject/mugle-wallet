...
use mugle_wallet_util::OnionV3Address;
	...
	K: Keychain,
{
	...
	///
	/// Each method will call the [`WalletBackend`](../mugle_wallet_libwallet/types/trait.WalletBackend.html)'s
	/// [`open_with_credentials`](../mugle_wallet_libwallet/types/trait.WalletBackend.html#tymethod.open_with_credentials)
	/// (initialising a keychain with the master seed,) perform its operation, then close the keychain
	/// with a call to [`close`](../mugle_wallet_libwallet/types/trait.WalletBackend.html#tymethod.close)
	///
	/// # Arguments
	...
	/// updates
	/// [`WalletBackend`](../mugle_wallet_libwallet/types/trait.WalletBackend.html) trait.
	///
	/// # Returns
	...
	/// # Returns
	/// * Result Containing:
	/// * A Vector of [`AcctPathMapping`](../mugle_wallet_libwallet/types/struct.AcctPathMapping.html) data
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Remarks
	///
	...
	/// ```
	/// # mugle_wallet_api::doctest_helper_setup_doc_env!(wallet, wallet_config);
	///
	/// let api_owner = Owner::new(wallet.clone(), None);
	///
	...
	/// # Returns
	/// * Result Containing:
	/// * A [Keychain Identifier](../mugle_keychain/struct.Identifier.html) for the new path
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Remarks
	///
	...
	/// a node (via the [`NodeClient`](../mugle_wallet_libwallet/types/trait.NodeClient.html)
	...
	/// [OutputCommitMapping](../mugle_wallet_libwallet/types/struct.OutputCommitMapping.html)
	/// of which each element is a mapping between the wallet's internal
	/// [OutputData](../mugle_wallet_libwallet/types/struct.Output.html)
	...
	/// Returns a list of [Transaction Log Entries](../mugle_wallet_libwallet/types/struct.TxLogEntry.html)
	...
	/// the given [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html) uuid.
	///
	/// # Returns
	...
	/// [TxLogEntries](../mugle_wallet_libwallet/types/struct.TxLogEntry.html)
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	///
	/// # Returns
	/// * (`bool`, [`WalletInfo`](../mugle_wallet_libwallet/types/struct.WalletInfo.html)) - A tuple:
	/// * The first `bool` element indicates whether the data was successfully
	/// refreshed from the node (note this may be false even if the `refresh_from_node`
	/// argument was set to `true`.
	/// * The second element contains the Summary [`WalletInfo`](../mugle_wallet_libwallet/types/struct.WalletInfo.html)
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	/// [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html) object containing
	...
	/// to the recipient).
	///
	/// If the `send_args` [`InitTxSendArgs`](../mugle_wallet_libwallet/types/struct.InitTxSendArgs.html),
	/// of the [`args`](../mugle_wallet_libwallet/types/struct.InitTxArgs.html), field is Some, this
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `args` - [`InitTxArgs`](../mugle_wallet_libwallet/types/struct.InitTxArgs.html),
	/// transaction initialization arguments. See struct documentation for further detail.
	///
	/// # Returns
	/// * a result containing:
	/// * The transaction [Slate](../mugle_wallet_libwallet/slate/struct.Slate.html),
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `args` - [`IssueInvoiceTxArgs`](../mugle_wallet_libwallet/types/struct.IssueInvoiceTxArgs.html),
	/// invoice transaction initialization arguments. See struct documentation for further detail.
	///
	/// # Returns
	/// * ``Ok([`slate`](../mugle_wallet_libwallet/slate/struct.Slate.html))` if successful,
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `slate` - The transaction [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html). The
	...
	/// [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html),
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `slate` - The transaction [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html). All
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `tx` - A completed [`Transaction`](../mugle_core/core/transaction/struct.Transaction.html),
	/// typically the `tx` field in the transaction [`Slate`](../mugle_wallet_libwallet/slate/struct.Slate.html).
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `tx_id` - If present, cancel by the [`TxLogEntry`](../mugle_wallet_libwallet/types/struct.TxLogEntry.html) id
	/// for the transaction.
	///
	...
	///
	/// # Returns
	/// * Ok(Some([Slate](../mugle_wallet_libwallet/slate/struct.Slate.html)) containing the stored
	...
	/// * [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	/// [TxLogEntry](../mugle_wallet_libwallet/types/struct.TxLogEntry.html) is created.
	///
	/// # Arguments
	///
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	...
	///
	/// # Returns
	/// * Ok with a  [`NodeHeightResult`](../mugle_wallet_libwallet/types/struct.NodeHeightResult.html)
	...
	/// `mugle-wallet.toml` file and the `wallet_data` directory that contains the wallet
	...
	///
	/// 1) The current directory, from which `mugle-wallet` or the main process was run, if it
	/// contains a `mugle-wallet.toml` file.
	/// 2) ~/.mugle/<chaintype>/ otherwise
	///
	/// # Arguments
	///
	/// * None
	...
	/// Create a `mugle-wallet.toml` configuration file in the top-level directory for the
	/// specified chain type.
	/// A custom [`WalletConfig`](../mugle_wallet_config/types/struct.WalletConfig.html)
	/// and/or mugle `LoggingConfig` may optionally be provided, otherwise defaults will be used.
	///
	...
	/// # mugle_wallet_api::doctest_helper_setup_doc_env!(wallet, wallet_config);
	///
	/// use mugle_core::global::ChainTypes;
	///
	/// let dir = "path/to/wallet/dir";
	///
	/// # let dir = tempdir().map_err(|e| format!("{:#?}", e)).unwrap();
	...
	/// path-related values in the optional configuration structs will be ignored.
	///
	/// The wallet files must not already exist, and ~The `mugle-wallet.toml` file must exist
	...
	///
	/// # Returns
	/// * Ok with a Vec of [`StatusMessage`](../mugle_wallet_libwallet/api_impl/owner_updater/enum.StatusMessage.html)
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	///
	/// # Returns
	/// * Ok with a [Slate](../mugle_wallet_libwallet/slate/struct.Slate.html) if successful
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	///
	/// # Returns
	/// * Ok with a [Slatepack](../mugle_wallet_libwallet/slatepack/types/struct.Slatepack.html) if successful
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	/// Returns a single, exportable [PaymentProof](../mugle_wallet_libwallet/api_impl/types/struct.PaymentProof.html)
	...
	///
	/// # Returns
	/// * Ok([PaymentProof](../mugle_wallet_libwallet/api_impl/types/struct.PaymentProof.html)) if successful
	/// * or [`libwallet::Error`](../mugle_wallet_libwallet/struct.Error.html) if an error is encountered
	...
	/// Verifies a [PaymentProof](../mugle_wallet_libwallet/api_impl/types/struct.PaymentProof.html)
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `proof` A [PaymentProof](../mugle_wallet_libwallet/api_impl/types/struct.PaymentProof.html))
	///
	/// # Returns
